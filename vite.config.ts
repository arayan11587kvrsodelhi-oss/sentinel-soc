...
function figmaSiteConfiguration(
  ...
  const title =
    config.title ?? 'SENTINEL SOC — Security Operations Center'
...
/**
 * Reload when a module that previously defined a React Refresh
 * boundary stops defining one.
 */
...
/**
 * Serves a blank render-target page at
 * /.figma/make/kit.html that the Figma preview script
 * drives directly.
 *
 * Dev-only: apply: 'serve' gates the plugin to Vite dev.
 */
...
export const stories = import.meta.glob(
  ${JSON.stringify(storiesGlob)}
)
`
