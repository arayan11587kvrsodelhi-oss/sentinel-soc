  ...
  return (
    <div className="space-y-5">
      ...
      >
        ...
        <div>
          <div className="flex items-center gap-2">
            <span className="text-base font-semibold" style={{ color: "#42D392" }}>
              ● HEALTHY
            </span>
          ...
          <div className="flex items-center gap-4 mt-1 text-xs" style={{ color: "#627083" }}>
      ...
      <div className="grid grid-cols-6 gap-3">
        {metrics.map((m) => (
          <div key={m.label} className="rounded-xl p-3" style={{ background: "#111925", border: "1px solid #1D2938" }}>
            <div className="text-[10px] font-semibold uppercase tracking-wider mb-1" style={{ color: "#627083" }}>
              {m.label}
            </div>
            <div className="text-xl font-semibold font-mono tabular-nums" style={{ color: "#F4F7FA" }}>
              {m.value}
            </div>
            ...
            >
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-mono text-xs font-medium" style={{ color: "#42D392" }}>
                    {sys.latency}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-mono text-xs font-medium" style={{ color: "#F4F7FA" }}>
                    {sys.uptime}
                  </span>
                </div>
                {(sys as { requests?: string }).requests && (
                  <div className="flex items-center justify-between">
                    <span className="text-xs" style={{ color: "#627083" }}>Requests</span>
                    <span className="font-mono text-xs" style={{ color: "#9AA8B8" }}>{(sys as { requests?: string }).requests}</span>
                  </div>
                )}
                {(sys as { connections?: string }).connections && (
                  <div className="flex items-center justify-between">
                    <span className="text-xs" style={{ color: "#627083" }}>Connections</span>
                    <span className="font-mono text-xs" style={{ color: "#9AA8B8" }}>{(sys as { connections?: string }).connections}</span>
                  </div>
                )}
                {(sys as { queries?: string }).queries && (
                  <div className="flex items-center justify-between">
                    <span className="text-xs" style={{ color: "#627083" }}>Queries</span>
                    <span className="font-mono text-xs" style={{ color: "#9AA8B8" }}>{(sys as { queries?: string }).queries}</span>
                  </div>
                )}
                {(sys as { model?: string }).model && (
                  <div className="flex items-center justify-between">
                    <span className="text-xs" style={{ color: "#627083" }}>Model</span>
                    <span className="font-mono text-xs" style={{ color: "#9AA8B8" }}>{(sys as { model?: string }).model}</span>
                  </div>
                )}
                {(sys as { throughput?: string }).throughput && (
                  <div className="flex items-center justify-between">
                    <span className="text-xs" style={{ color: "#627083" }}>Throughput</span>
                    <span className="font-mono text-xs" style={{ color: "#9AA8B8" }}>{(sys as { throughput?: string }).throughput}</span>
                  </div>
                )}
                {(sys as { evaluations?: string }).evaluations && (
                  <div className="flex items-center justify-between">
                    <span className="text-xs" style={{ color: "#627083" }}>Evaluations</span>
                    <span className="font-mono text-xs" style={{ color: "#9AA8B8" }}>{(sys as { evaluations?: string }).evaluations}</span>
                  </div>
                )}
              </div>

              <div className="mt-3 pt-2" style={{ borderTop: "1px solid #1D293840" }}>
      ...
      <div className="rounded-xl p-4" style={{ background: "#111925", border: "1px solid #1D2938" }}>
        ...
        <div className="flex items-center gap-6">
          {[
            { status: "OPERATIONAL", color: "#42D392", desc: "Fully functional" },
            { status: "DEGRADED", color: "#FF8A4C", desc: "Reduced performance" },
            { status: "OFFLINE", color: "#FF4D5E", desc: "Service unavailable" },
            { status: "RECONNECTING", color: "#F4C95D", desc: "Attempting reconnection" },
          ].map((s) => (
