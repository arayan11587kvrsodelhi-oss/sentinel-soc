...
const configs: Record<
  ProvenanceType,
  { icon: string; label: string; color: string; classification: string; defaultSource: string; defaultTransformation: string }
> = {
  live: {
    icon: "●",
    label: "LIVE",
...
export function ProvenanceBadge({ type, source, retrieved, transformation }: ProvenanceBadgeProps) {
      ...
      {open && (
        ...
        >
          <div className="flex items-center justify-between mb-2">
            ...
            <button onClick={() => setOpen(false)} className="text-[10px]" style={{ color: "#627083" }}>
              ✕
            </button>
          </div>
