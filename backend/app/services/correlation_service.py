...
class CorrelationEngine:
    ...
    def _init_sqlite(self):
        ...
        try:
            with self._get_db_conn() as conn:
                ...
                for col in ("source_ips", "affected_targets", "attack_stage", "first_seen", "last_seen"):
                    try:
            ...
            with self._get_db_conn() as conn:
                conn.execute("""
                    INSERT INTO incidents (
                        ...
                        source_ips, affected_targets, attack_stage, first_seen, last_seen
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ...
    def process_event(self, event: SecurityEvent) -> Optional[Incident]:
        ...
        # 1. Correlate with existing open incident matching scenario_id, or source_ip + category/target
        existing_inc: Optional[Incident] = None
        for inc in self._incidents.values():
            if inc.status in ("OPEN", "INVESTIGATING"):
                # Exact scenario match
                if event.scenario_id and any(
                    e.scenario_id == event.scenario_id for e in self._events_buffer if e.event_id in inc.event_ids
                ):
                    existing_inc = inc
                    break
                # Same attacker IP
                if event.source_ip == inc.source_ip or (inc.source_ips and event.source_ip in inc.source_ips):
                    existing_inc = inc
                    break
                # Same target under active exploit/ransomware
        ...
        if existing_inc:
            ...
            existing_inc.last_seen = now_str
        ...
        # 2. Rule: Brute Force / Credential Abuse
        if event.event_type in ("BRUTE_FORCE", "LOGIN_FAILURE"):
            ...
            if len(matching_events) >= 3:
                ...
                new_inc = Incident(
                    ...
                    severity="CRITICAL" if any(e.event_type == "BRUTE_FORCE" for e in matching_events) else "HIGH",
                    status="OPEN",
                    confidence=0.95,
                    category="Credential Access",
        ...
        # 3. Rule: Web Exploit / Vulnerability Chain
        ...
        # 4. Rule: PowerShell / Defense Evasion / Privilege Escalation
        ...
        # 5. Rule: Ransomware / Lateral Movement Chain
        ...
        # 6. Rule: Data Exfiltration Chain
